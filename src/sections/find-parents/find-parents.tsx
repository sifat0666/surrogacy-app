...
const FindParents = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      if (authUserToken) {
        ...
        setData(data?.intended_parents);
        setOriginalData(data?.intended_parents);
      }
      else {
  ...
  const handleFormValues = (values: any) => {
    ...
    const keywords = {
      ...
      surrogateLocation: values.surrogateLocation || "",
    ...
    if (allValuesEmptyOrFalse) {
      ...
      const filteredData = data?.filter((surrogate: any) => {
        return (
          ...
          (!keywords.surrogateLocation || (surrogate?.surrogate_location && surrogate.surrogate_location.toLowerCase().includes(keywords.surrogateLocation.toLowerCase()))) &&
  ...
  return (
    <>
      <ResourceFinder
        ...
        loadMoreButtonColor="warning.main"
        loading={isLoading}
