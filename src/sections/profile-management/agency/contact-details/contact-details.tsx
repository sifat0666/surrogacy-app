...
const ContactDetails = () => {
  ...
  const methods = useForm<any>({
    resolver: yupResolver(
      Yup.object({
        // email: Yup.string().required("Required Fields!"),
        // phone: Yup.string().required("Required Fields!"),
        // website: Yup.string().required("Required Fields!"),
      })
    ),
    defaultValues: {
  ...
  const onSubmit = async (values: any) => {
    ...
    if (authUser?.stripe_date?.active) {
      setLoading(true);
      try {
        await mutation({
